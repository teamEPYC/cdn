console.log("Button-animated-strokes.js loaded")
...
document.querySelectorAll('[data-animatedStroke="true"]').forEach(targetEl => {
  ...
  const config = {
    strokeWidth: parseFloat(targetEl.getAttribute("data-animatedStroke-width")) ||
      animatedStrokeConfig.strokeWidth,
    duration: parseFloat(targetEl.getAttribute("data-animatedStroke-duration")) ||
      animatedStrokeConfig.duration,
    delay: parseFloat(targetEl.getAttribute("data-animatedStroke-delay")) ||
      animatedStrokeConfig.delay,
    easing: targetEl.hasAttribute("data-animatedStroke-easing") ?
      targetEl.getAttribute("data-animatedStroke-easing") : animatedStrokeConfig.easing,
    color: targetEl.getAttribute("data-animatedStroke-color") || animatedStrokeConfig.color,
    segmentLength: parseInt(targetEl.getAttribute("data-animatedStroke-segmentLength")) ||
      animatedStrokeConfig.segmentLength
  };
...
function createAnimatedStrokeCanvas(targetEl, config) {
  ...
  function drawSegment(segmentPoints) {
    if (!segmentPoints.length || !segmentPoints[0] || !segmentPoints[segmentPoints.length - 1])
      return;
