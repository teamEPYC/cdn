window.addEventListener("DOMContentLoaded", function () {
  const isDebug = localStorage.getItem("team-EPYC-debugger");
  ...
  if (!isDebug) {
          ...
          if (rules) {
            if (type && type === "radio") {
              ...
              if (name === "Agree-to-Terms") {
                const checkboxParent = $(field).parents(
    ...
    function toggleSubmitButtonVisibility(stepNumber) {
      ...
      if (submitButton) {
        submitButton.style.display =
          stepNumber === steps.length ? "flex" : "none";
      }
    }
    ...
    function updateTextAreaWordCount(target) {
      ...
      if (counter) {
        counter.text(
          `Word Limit: ${
            target.value ? target.value.split(/[\s]+/).length : 0
          }/100`
        );
      }
    }
