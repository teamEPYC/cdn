 ...
 *
 * r3 (Early Access page): the canvas is sized to the box that contains it (the
 * strip), not to the whole section. The section also holds the CTA row and is
 * wider than the inset strip, so r2 drew a canvas that the strip clipped on the
 * right and bottom, hiding those edge cubes. The locked edge frame scales with the
 * grid, measured against the 1440px strip grid (32 x 11 cells), so it shows evenly
 * on all four sides and thins to about one cell on a phone.
 */
...
(function () {
  ...
  function isLocked(x, y, cols, rows) {
    ...
    const sideScale = Math.min(1, cols / 32);
    const rowScale = Math.min(1, rows / 11);
