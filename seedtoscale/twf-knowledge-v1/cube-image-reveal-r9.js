 ...
 * r9: the sweep runs along the title's first line, centred in one row of cubes, so
 * the trail is a single row (it no longer straddles two rows or the gap between lines).
 */
...
(function () {
  ...
  function initCubeReveal(section) {
    ...
    function hintTrack() {
      ...
      const mid = box ? (box.lineTop + box.lineBottom) / 2 : height / 2;
      const row = Math.max(0, Math.min(rows - 1, Math.floor(mid / cellH)));
      const y = (row + 0.5) * cellH;
      const lift = cellH * CONFIG.hint.arc;
    ...
    function measureTitle(box) {
      ...
      if (!text.width || !text.height) return null;
      const line =
        Array.prototype.find.call(range.getClientRects(), (r) => r.width > 0 && r.height > 0) ||
        text;
      return {
