(function(){
  ...
  /* r5: the report opens itself once the form succeeds, so nobody has to press
     a second button. Webflow reveals .w-form-done by flipping its inline
     display, which fires no event, so this watches the node instead.

     window.open from an async success callback is outside the click gesture
     and some browsers will block it. That is why the fallback link stays on
     screen and is never hidden: if the open is refused, the reader still has
     one obvious way through. The intro line is hidden at the same moment
     because it asks where to send something that has already arrived. */
  function initReportDelivery(){
    ...
    var link=done.querySelector('a[href]');
    var fired=false;
    function shown(){
