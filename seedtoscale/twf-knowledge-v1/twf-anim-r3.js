  ...
  function init() {
    ...
    var SCENES = [
      ...
      { host: '.twf_cards', steps: [
        // r3: the '.twf_card' NUDGE and the '.twf_card-img' ZOOM are both removed.
        // library.js owns the library row now - it runs the scroll fly-in (cards 1 and 3
        // from the sides, card 2 dropping in) and the hover fade. Two scripts writing a
        // transform to the same .twf_card fought each other, which is why the cards
        // settled at staggered offsets and escaped their media boxes. The card-img step
        // is dead regardless: those placeholder images were removed from the markup.
      ...
      // r3: host is the TRACK, not the pinned panel. The About section is now
      // position:sticky inside a 150vh track. A ScrollTrigger whose trigger element is
      // inside the pinned panel never advances - once pinned, the panel's position
      // relative to the viewport stops changing, so `start` is evaluated against a box
      // that no longer moves and the reveals never fire. The track keeps scrolling, so
      // it is the correct trigger. Single selector only: hosts are resolved with
      // querySelectorAll and every match builds its own timeline, so a comma listing
      // both the track and '.twf_about' would animate the same copy twice.
      { host: '.twf_about-track', steps: [
            ...
            if (st.split) {
              if (hasSplit) {
                ...
                els.forEach(function (el, i) {
                  // r2: the hover-fill button component ([data-button]) is a positioned
                  // composite, not a label plus a trailing icon. Its fill is absolutely
                  // positioned against the <a>; SplitText would re-parent it into a line
                  // wrapper, which both moves the fill's containing block and clips it to
                  // the mask's overflow:clip - so the circle grows inside the text strip
                  // instead of across the button. Leave these unsplit. The separate block
                  // step ({ from: yPercent 100, opacity 0 }) still travels the button in,
                  // and it owns opacity here because this split step is `also: true`.
                  if (el.hasAttribute && el.hasAttribute('data-button')) return;
