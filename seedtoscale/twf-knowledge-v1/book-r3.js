(() => {
  ...
  function drawStamp(ctx, x, y, rotation, label = "IN PROGRESS") {
    ...
    const w = Math.max(px(560), px(64) + [...label].length * px(38));
    const h = px(150);
    const color = "#FF3621";
    ctx.strokeStyle = color;
    ctx.fillStyle = "rgba(255, 54, 33, 0.07)";
    ctx.lineWidth = px(13);
    roundedRect(ctx, -w / 2, -h / 2, w, h, px(16));
    ctx.fill();
    ctx.stroke();
    ctx.lineWidth = px(3);
    roundedRect(ctx, -w / 2 + px(17), -h / 2 + px(17), w - px(34), h - px(34), px(9));
    ctx.stroke();
    ctx.fillStyle = color;
    ctx.font = avenir(500, px(46));
    fillSpacedText(ctx, label, 0, px(2), px(8));
  ...
  function drawFrontierArt(ctx, w) {
    ...
    const palette = ["#F0B49B", "#E79A7E", "#DFAF98", "#F2C9B6", "#D98F72", "#E8D3C2"];
    for (let i = 0; i < cols; i++) {
      for (let j = 0; j < cols; j++) {
        ...
        ctx.globalAlpha = 0.58 + falloff * 0.42;
  ...
  function drawTeamsArt(ctx, w) {
    ...
    for (let i = 0; i < cols; i++) {
      for (let j = 0; j < cols; j++) {
        ...
        if (grain > blob * 1.18 + 0.30) continue;
        ctx.fillStyle = blob > 0.45 ? CORAL : grain > 0.55 ? "#B3ADA2" : "#6C6862";
  ...
  // r2: unreleased books are stamp-only (client copy doc: stamp, no titles, no other copy).
