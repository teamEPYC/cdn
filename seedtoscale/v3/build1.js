...
(() => {
  ...
  var MultiStepFormManager = class {
    ...
    action_updateDataAndError() {
      ...
      console.log("[+] THIS STEP ERRORS", errors);
      this.updateState({
        formData: data,
        errors
      });
  ...
  var PROTECTED_PAGES = ["/onboarding", "/profile"];
  ...
  var ROUTES = {
    LOGIN: `https://${ENV.DOMAIN}/home-new`,
    ONBOARDING: `https://${ENV.DOMAIN}/onboarding`
  };
  ...
  var loginHandler = (auth0Client, elementId = "#login-button") => {
    ...
    if (elements && elements.length) {
      console.info("[+] Login Button Found. Installing listener");
      elements.forEach((element) => {
        console.info("[++] Listener Installed on [id]", element.id);
  ...
  var User = class {
    ...
    constructor() {
      ...
      __publicField(this, "updateMetaDataInLocalStorage", async (user_metadata) => {
        const data = localStorage.getItem("formState");
        if (!data) {
          return;
        }
        const state = JSON.parse(data);
        const newState = { ...state, formData: { ...state.formData, ...user_metadata } };
  ...
  window.Webflow.push(() => {
    ...
    function userLoaded(userObject) {
      ...
      user.updateMetaDataInLocalStorage(userObject.user_metadata);
      if (MSF) {
        MSF.initialize();
      }
    }
    ...
    if (form) {
      MSF = new MultiStepFormManager({
        ...
        inputErrorMessageSelector: ".onb-form-field-error",
        options: {
          afterSubmitRedrect: document.querySelector(formSelector)?.getAttribute("data-redirect") || "/"
        },
        onStepChange: (state) => {
